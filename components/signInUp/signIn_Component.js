...
const Signin = () => {
    ...
    const { setAuthAdmin } = useContext(AuthContext)

    const [login, { loading, error }] = useMutation(LOG_IN_USER, {
        variables: { ...AdminInfo },
        onCompleted: data => {
            if (data) {
                Cookies.set("jwt", data.loginAdmin.jwt)
                setAuthAdmin(data.loginAdmin.admin)
                setAdminInfo({
                    email: "",
                    password: ""
                })
                ...
                <div className="bg-green-300 pt-8 pb-16">
                    ...
                    <div className="w-4/5 mx-auto">
                        <div className="flex items-center bg-white rounded shadow-md mb-4">
                            ...
                            <input className="w-full h-12 focus:outline-none" type="email" name="email" placeholder="Email" value={AdminInfo.email} onChange={handleChange} />
                        </div>
                        <div className="flex items-center bg-white rounded shadow-md mb-4">
                            <span className="px-3">
                                <svg className="fill-current text-gray-500 w-4 h-4" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20">
                            ...
                            <input className="w-full h-12 focus:outline-none" type="password" name="password" placeholder="Password" value={AdminInfo.password} onChange={handleChange} />
                        </div>
